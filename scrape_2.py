...
# filename=input("Enter The filename here with extension ig. Numbers.csv  ")
print("Ensure that `numbers.txt` exist at same path as script")
def get_current_day():
        ...
        for row in sheet.iter_rows():
            for cell in row:
                ...
                if number.isdigit():
                    ...
                    print("Skipping non-numeric value:", number)
...
def process_xls(filename):
    try:
        ...
        print("Error processing XLS file:", e)
...
def select_file_and_process():
    ...
    if filename:
        ...
        if filename.endswith('.xlsx') or filename.endswith('.xls'):
            ...
            print("Unsupported file format. Please select an Excel (.xlsx, .xls) or CSV file (.csv).")
...
def scrape_details(number):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Further clean up the schedule to remove any leading or trailing commas or spaces
            schedule = schedule.strip().strip(',')
            returned_schedule = remove_day_from_schedule(schedule)
            returned_schedule = process_schedule(returned_schedule)
            print(returned_schedule)
            reviews = remove_reviews_word(reviews)
            translated_name = translate_to_japanese(name)
            translated_addr = translate_to_japanese(address)
            translated_rating = translate_to_japanese(rating)
            translated_reviews = translate_to_japanese(reviews)
            if "無" in returned_schedule:
                returned_schedule = returned_schedule
            else:
                returned_schedule = translate_to_japanese(returned_schedule)
            # print(translated_addr, translated_rating, translated_name, translated_reviews, translated_shedule)
            ...
            return {"番号": number, "名前": translated_name, "スケジュール": returned_schedule, "休日": rest_day, "評価": translated_rating, "レビュー": translated_reviews, "住所": translated_addr}
        else:
...
try:
    ...
    day=get_current_day()
    root = tk.Tk()
    root.title("ファイルの選択と処理")

    # Configure style for the widgets
    style = ttk.Style()
    style.configure('TFrame', background='#e1d8b9')
    style.configure('TButton', background='#4caf50', foreground='white', font=('Helvetica', 12, 'bold'), padding=10)
    ...
    button_select = ttk.Button(frame, text="ブラウズ", command=select_file_and_process)
    button_select.grid(row=3, column=0, pady=20)
